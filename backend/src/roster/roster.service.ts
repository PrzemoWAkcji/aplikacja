...
export class RosterService {
    ...
    async importEntriesFromCsv(meetingId: string, csvContent: string): Promise<{ imported: number; updated: number }> {
        ...
        if (!entries || entries.length === 0) {
            throw new BadRequestException('Brak danych do zaimportowania lub nieprawidłowy format pliku CSV.');
        }
        ...
        for (const row of entries) {
            try {
                ...
                if (isRelay(row.eventCode || '', event.name || '')) {
                    ...
                    const club = (row as any).clubName || (row as any).shortClubName || 'Unattached';
                ...
                const entryData = {
                    athleteName: (row as any).fullName || `${(row as any).firstName || ''} ${(row as any).lastName || ''}`.trim(),
                    ...
                    birthDate: (row as any).dateOfBirth ? new Date((row as any).dateOfBirth) : null,
                    yearOfBirth: (row as any).yearOfBirth ? parseInt((row as any).yearOfBirth.toString(), 10) : null,
                    ...
                    heat: row.heat || (row as any).eventGroup ? parseInt((row.heat || (row as any).eventGroup).toString(), 10) : null,
                    eventId: event.id,
                    status: EntryStatus.CONFIRMED,
                };

                if (existingEntry) {
                    const athlete = await this.entriesService.findOrCreateAthlete(entryData as any);
        ...
        for (const [key, members] of relayGroups) {
            try {
                ...
                const club = (firstRow as any).clubName || (firstRow as any).shortClubName || 'Sztafeta';
                ...
                const entryData = {
                    ...
                    heat: firstRow.heat || (firstRow as any).eventGroup ? parseInt((firstRow.heat || (firstRow as any).eventGroup).toString(), 10) : null,
                    eventId: event.id,
                    status: EntryStatus.CONFIRMED,
        ...
        for (const row of results) {
            try {
                ...
                const resultData = {
                    ...
                    placeGender: row.placeGender ? parseInt(row.placeGender.toString(), 10) : null,
                    time: row.result,
                    resultRounded: row.resultRounded,
                    windReading: row.windReading,
                    wind: row.windReading ? parseFloat(row.windReading.replace(',', '.')) : null,
    ...
    private mapGender(gender?: string): string {
        ...
        if (g === 'male' || g === 'm' || g === 'mężczyzna' || g === 'mezczyzna') return 'M';
    ...
    private generateCsv<T extends Record<string, any>>(data: T[]): string {
        ...
        return Papa.unparse({ fields: displayHeaders, data: data.map(row => Object.values(row)) });
    }
    ...
    private async findOrCreateEvent(meetingId: string, row: RosterEntryDto) {
        ...
        const baseName = row.eventName || row.pzlaEventCode || row.eventStart || `Konkurencja ${row.eventCode}`;
        ...
        const genderLabel = gender === 'M' ? 'Mężczyźni' : (gender === 'K' ? 'Kobiety' : 'Open');
        if (gender !== 'MIX' && !baseName.toLowerCase().includes(genderLabel.toLowerCase()) && !baseName.toLowerCase().includes(gender.toLowerCase())) {
        ...
        if (!event) {
            ...
            if (event.name.startsWith('Event ') || event.name === 'Unknown Event' || /^\d{4}-\d{2}/.test(event.name)) {
            ...
            if (row.eventStart) {
                ...
                if (!event.startTime || event.startTime.getTime() !== csvStartTime.getTime()) {
    ...
    private findBestResult(rounds: (string | undefined)[]): string | null {
        ...
        return validResults.length > 0 ? Math.max(...validResults).toString() : null;
    }
}
